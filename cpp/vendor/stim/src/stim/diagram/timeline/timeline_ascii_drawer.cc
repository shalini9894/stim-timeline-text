...
void DiagramTimelineAsciiDrawer::do_single_qubit_gate_instance(const ResolvedTimelineOperation &op) {
    ...
    std::stringstream ss;
    auto tsim_label = tsim_labels::logical_label(op.tag, gate_data.name);
    if (tsim_label.has_value()) {
        ss << *tsim_label;
    } else {
        ss << gate_data.name;
    }
...
void
 DiagramTimelineAsciiDrawer::do_correlated_error(const ResolvedTimelineOperation &op) {
    if (cur_moment_is_used) {
        start_next_moment();
    }
